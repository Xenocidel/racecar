...
def calculateSide(in_dir,out_dir,current_node_index):           #Input: 'in', 'out' directions (0,1,2,3) and current node index (from list 'nodes')
...
def findSide():                                                 #Functionality: generates current side to follow using earlier functions
    global currentNode, car_x, car_y                                #Global Variables needed to call inner functions
...
in_threshold=6                  #Threshold for how close to the node you must be to set the wall
